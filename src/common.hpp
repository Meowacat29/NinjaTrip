...
#define gold_coin_path(name) data_path "/goldCoin/" name
#define audio_path(name) data_path  "/audio/" name
#define mesh_path(name) data_path  "/meshes/" name
#define level_path(name) data_path  "/levelDesign/" name
#define ui_path(name)  data_path "/textures/ui/" name
#define score_path(name)  data_path "/textures/ui/scores/" name
#define health_path(name) data_path "/textures/ui/health/" name
// Not much math is needed and there are already way too many libraries linked (:
// If you want to do some overloads..
struct vec2 { float x, y; };
struct vec2Int { int x, y; };  // new struct added for matrix coordinate processing in World::level_display()
