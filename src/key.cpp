...
bool Key::init()
{
	...
	{
		if (!key_texture.load_from_file(textures_path("key.png")))
		{
	...
	float wr = key_texture.width * SIZE_MODIFIER;
	float hr = key_texture.height *SIZE_MODIFIER;
...
vec2 Key::get_bounding_box()const
{
	...
	return { std::fabs(m_scale.x) * key_texture.width, std::fabs(m_scale.y) * key_texture.height };
}